...
BASEDIR=$(dirname $0)
echo "------ PURCHASE INVOICE VALIDATION -----"
echo 'install module'
python $BASEDIR/../module_install.py -d $DATABASE -u $USER -w $PASSWORD -s $URL purchase_invoice_validation

echo 'migration.sql'
psql -h $DB_HOST -d $DATABASE -U $DB_USER -f $BASEDIR/migration.sql
echo "------ PURCHASE INVOICE VALIDATION (FIN) -----"