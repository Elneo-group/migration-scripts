...
python $BASEDIR/../module_install.py -d $DATABASE -u $USER -w $PASSWORD -s $URL partner_nace
echo 'Update nace list...'
python $BASEDIR/import_nace.py -d $DATABASE -u $USER -w $PASSWORD -s $URL
echo 'Transfert data to new modules...'
psql -h $DB_HOST -d $DATABASE -U $DB_USER -f $BASEDIR/migration.sql
echo "------ HR PARTNER_NACE (FIN) -----"